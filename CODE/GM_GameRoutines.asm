...
;
    ...
    OR A
    LD A, $00
    JR NZ, ChkStPos                     ;if water type, set swimming flag, otherwise do not set
    INC A
ChkStPos:
    LD (SwimmingFlag), A
;
...
;
    ...
    OR A
    JR Z, ChkOverR                      ;if set to zero, branch (do not use dummy byte for this)
    LD A, (FetchNewGameTimerFlag)       ;do we need to set the game timer? if not, use 
    OR A
    JR Z, ChkOverR                      ;old game timer setting
;
    LD DE, GameTimerData
...
Vine_AutoClimb:
    ...
    OR A
    JR NZ, AutoClimb                    ;above the status bar yet and if so, set modes
...
VerticalPipeEntry:
    ...
    OR A
    JR NZ, ChgAreaPipe                  ;if set, branch to use mode 0
    INC C
...
FlagpoleSlide:
    ...
    JR NZ, NoFPObj                      ;if not found, branch to something residual
    ...
    LD A, $00
    JR NC, SlidePlayer                  ;far enough, and if so, branch with no controller bits set
...
PlayerEndLevel:
    ...
    CP A, $AE
    JR C, ChkStop                       ;if player is not yet off the flagpole, skip this part
    LD A, (ScrollLock)                  ;if scroll lock not set, branch ahead to next part
    OR A
    JR Z, ChkStop                       ;because we only need to do this part once
...
ChkStop:
    ...
    JR C, RdyNextA                      ;if d0 set, skip to next part
    LD A, (StarFlagTaskControl)         ;if star flag task control already set,
    OR A
    JR NZ, InCastle                     ;go ahead with the rest of the code
    INC A
...
RdyNextA:
    ...
    JR NZ, NextArea                     ;and skip this last part here if not
    ...
    JR C, NextArea                      ;at least this number of coins, leave flag clear
...
StillInGame:
    ...
    JR Z, GetHalfway                    ;leave offset alone
    INC B
GetHalfway:
    LD A, B
...
MaskHPNyb:
    ...
    CP A, (HL)
    JR Z, SetHalfway                    ;left side of screen must be at the halfway page,
    JR C, SetHalfway                    ;otherwise player must start at the
...
EntrMode2:
    ...
    OR A
    JR NZ, VineEntr                     ;branch to enter with vine
    ...
    JR C, PlayerRdy                     ;to be at specific height to look/function right) branch
...
VineEntr:
    ...
    JR C, OffVine                       ;if vertical coordinate < preset value, use defaults
    LD A, $03
...
SaveJoyp:
    ...
    JR Z, SizeChk                       ;if not, branch
    LD A, (Player_State)                ;check player's state
    OR A
    JR NZ, SizeChk                      ;if not on the ground, branch
    LD A, (Left_Right_Buttons)          ;check left and right
    OR A
    JR Z, SizeChk                       ;if neither pressed, branch
    XOR A
...
SizeChk:
    ...
    LD A, (CrouchingFlag)
    OR A
    JR Z, ChkMoveDir                    ;if not, branch ahead
...
ChkMoveDir:
    ...
    JR Z, PlayerSubs                    ;if not moving at all horizontally, skip this part
...
PlayerSubs:
    ...
    JR C, PlayerHole                    ;if so, branch ahead
...
PlayerInjuryBlink:
    ...
    JR NC, ExitBlink                    ;branch if before that point
    CP A, $C8                           ;check again for another specific point
    JR Z, DonePlayerTask                ;branch if at that point, and not before or after
...
PlayerFireFlower:
    ...
    JR Z, ResetPalFireFlower            ;branch if at moment, not before or after
...
;
    ...
    LD C, A                 
    JR C, ChkBrick                  ;if player is small, use metatile itself as contents of A
    LD C, MT_BLANK                  ;otherwise init metatile
ChkBrick:
    JR NZ, PutMTileB                ;if no match was found in previous sub, skip ahead
;
...
;
    ...
    JR Z, StartBTmr                 ;if so, branch
    CP A, MT_BRICK_COIN             ;is it brick with coins (without line)?
    JR NZ, PutMTileB                ;if not, branch ahead to store empty block metatile
;
StartBTmr:
    LD C, A
    LD A, (BrickCoinTimerFlag)      ;check brick coin timer flag
    OR A
    JR NZ, ContBTmr                 ;if set, timer expired or counting down, thus branch
;
    LD A, $0B
...
ContBTmr:
    ...
    OR A
    JR NZ, PutMTileB                ;if not yet expired, branch to use current metatile
...
;
    ...
    LD A, (HL)
    CP A, $11
    JR Z, Unbreak                   ;if set to value loaded for unbreakable, branch
...
;
    ...
    JR C, BlockCode                 ;branch to use current number
...
PlayerMovementSubs:
    ...
    JR NZ, SetCrouch                    ;if so, branch
;
    LD A, (Player_State)                ;check state of player
    OR A
    JR NZ, ProcMove                     ;if not on the ground, branch
;
...
;
    ...
    CP A, $03
    JR Z, MoveSubs                      ;if climbing, branch ahead, leave timer unset
...
;
    ...
    OR A
    JR Z, GndMove                       ;if left/right controller bits not set, skip instruction
...
;
    ...
    JR NZ, ProcSwim                     ;if so, branch elsewhere
;
    ...
    JR C, ProcSwim                      ;or just starting to jump, if just starting, skip ahead
DumpFall:
...
LRWater:
    ...
    OR A
    JR Z, LRAir                         ;if not pressing any, skip
...
;
    ...
    AND A, (HL)
    JR Z, InitCSTimer                   ;if not set, skip to end
;
    LD E, A                             ;save to E
...
;
    ...
    JR C, ClimbFD                       ;if controller right pressed, branch ahead
    INC L                               ;otherwise increment offset by 2 bytes
    INC L
ClimbFD:
    DEC A                               ;check to see if facing right
    JR Z, CSetFDir                      ;if so, branch, do not increment
...
PlayerPhysicsSub:
    ...
    CP A, $03
    JR NZ, CheckForJumping              ;if not climbing, branch
;
;   CLIMBING PHYSICS
;
    ...
    JR Z, ProcClimb                     ;if not pressing up or down, branch
;
    INC E
...
ProcJumping:
    LD A, (Player_State)                ;check player state
    OR A
    JR Z, InitJS                        ;if on the ground, branch
;
...
;
    ...
    .IF PALBUILD == $00
    CP A, $09
    JR C, ChkWtr                        ;branch if below certain values, increment Y
    ...
    JR C, ChkWtr                        ;note that for jumping, range is 0-4 for Y
    INC E
ChkWtr:
...
;
    ...
    CP A, $14
    JR NC, X_Physics                    ;if below a certain point, branch
...
;
    ...
    JR NC, GetXPhy                      ;if =>$19 branch here
    JR C, ChkRFast                      ;if not branch elsewhere
ProcPRun:
    INC E                               ;if mario on the ground, increment Y
    LD A, (AreaType)                    ;check area type
    OR A
    JR Z, ChkRFast                      ;if water type, branch
...
;
    ...
    JR C, GetXPhy                       ;if less than a certain amount, branch ahead
FastXSp:
...
GetXPhy:
    ...
    JR NZ, GetXPhy2                     ;if not running, skip and use old value of Y
...
GetPlayerAnimSpeed:
    ...
    JR NC, SetRunSpd                    ;if greater than a certain amount, branch ahead
;
    ...
    JR NC, ChkSkid                      ;if greater than this but not greater than first, skip increment
    INC E                               ;otherwise increment Y again
ChkSkid:
    LD A, (SavedJoypadBits)             ;get controller bits
    AND A, %01111111                    ;mask out A button
    JR Z, SetAnimSpd                    ;if no other buttons pressed, branch ahead of all this
;
    AND A, $03                          ;mask out all others except left and right
    LD HL, Player_MovingDir
    CP A, (HL)                          ;check against moving direction
    JR NZ, ProcSkid                     ;if left/right controller bits <> moving direction, branch
...
ProcSkid:
    ...
    JR NC, SetAnimSpd                   ;if greater than this amount, branch
;
    LD A, (PlayerFacingDir)
...
ImposeFriction:
    ...
    AND A, (HL)
    JR NZ, JoypFrict                    ;if any bits set, branch to next part
;
    LD A, (Player_X_Speed)
    OR A
    JR Z, SetAbsSpd                     ;if player has no horizontal speed, branch ahead to last part
...
JoypFrict:
    ...
    JR NC, RghtFrict                    ;if left button pressed, carry = 0, thus branch
LeftFrict:
