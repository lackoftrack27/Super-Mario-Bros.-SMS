...
NonMaskableInterrupt:
    LD A, (Mirror_VDP_REG1)         ;disable NMIs and display in mirror reg
    AND A, %10011111                ;save all other bits
...
TileStreamRet:
    ...
    ;CALL SoundEngine                ;play sound
    CALL ReadJoypads                ;read joypads
    CALL PauseRoutine               ;handle pause
    CALL UpdateTopScore
;
...
RotPRandomBit:
    ...
    DJNZ RotPRandomBit              ;decrement for loop
;   SPRITE 0 FLAG CHECK
    LD A, (Sprite0HitDetectFlag)    ;check for flag here (don't wait for sprite 0 if flag isn't set)
    OR A
    JP Z, SkipSprite0
;   PAUSE CHECK
Sprite0Clr:
    LD A, (GamePauseStatus)         ;if in pause mode, do not bother with sprites at all
    RRA
    JP C, Sprite0Hit
    CALL MoveSpritesOffscreen
    CALL SpriteShuffler
;   WAIT FOR SPRITE 0 (SCANLINE 7)
Sprite0Hit:
    IN A, (VDPLINE_PORT)            ;do sprite #0 hit detection
    IN A, (VDPLINE_PORT)
    CP A, $07
    JP NZ, Sprite0Hit
;   SET SCROLL POSITION
SkipSprite0:    
    LD A, (HorizontalScroll)        ;set scroll registers from variables
    NEG
    OUT (VDPCON_PORT), A
    LD A, $88
    OUT (VDPCON_PORT), A
;   MAIN GAME EXEC
    ...
    LD A, (GamePauseStatus)         ;if in pause mode, do not perform operation mode stuff
    RRA
    CALL NC, OperModeExecutionTree

;   REACTIVATE NMIs
    LD A, (Mirror_VDP_REG1)
    OR A, %10100000
    OUT (VDPCON_PORT), A
    LD A, $81
    OUT (VDPCON_PORT), A
    IN A, (VDPCON_PORT)             ;clear any pending VDP interrupts
    EI                              ;enable Z80 interrupts
    RET                             ;we are done until the next frame!
    */
...
;   Do appropriate mode (horizontal)
    ...
    /*
    JP HoriWriteMode_W          ; 54
VertWritePrep:
;   Do additional prep for vertical writes
    DEC HL                      ;get low byte of vdp address and store in HL'
    LD A, (HL)
    EXX
    LD L, A
    EXX
    DEC HL                      ;get high byte of vdp address and store in HL'
    LD A, (HL)
    EXX
    LD H, A
    LD DE, $0040                ;address stride in DE' (1 col)
    LD C, VDPCON_PORT           ;VDP control port in C'
    EXX
    INC HL
    INC HL
;
    ...
    INC HL
    BIT 7, A
    JP NZ, VertWriteMode_W
VertWriteMode_B:    ; OVERHEAD: 39 + 92 = 131
    AND A, %00111111
;   E = VAL
    LD E, A
;   BC = VAL * 8
...
;   VAL = -(BC + E)
    ...
    addAToBC_M              ; 20
    negBC_M                 ; 24
    LD IY, WriteVeriBlock_B
    ADD IY, BC
    LD C, VDPDATA_PORT
    JP (IY)
VertWriteMode_W:    ; OVERHEAD: 55 + 92 = 147
    AND A, %00111111
    LD D, A
;   E = VAL * 3
    ADD A, A
